...
>
  <button
    ...
    class="w-full select-none relative before:absolute before:-top-1 before:-right-1 before:-bottom-1.5 before:left-7 before:-z-1 before:rounded-lg hover:before:bg-muted before:duration-300"
  >
    <div class="flex items-center z-1 mb-1 gap-3">
      <div
        class="flex items-center justify-center size-6 rounded-md bg-gray-700 border border-gray-300"
      >
        <Code class="size-4" />
      </div>
      ...
      <div transition:fade={{ duration: 300 }}>
        ...
        <ul class="flex flex-wrap gap-1.5 pt-2 pl-9 text-xs">
          {#each position.skills as skill}
            <li class="flex items-center border rounded-lg p-1.5 gap-1">
              <DevIcon name={skill} />
              <span>{skill}</span>
            </li>
          {/each}
